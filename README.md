...
| <a name="input_parameters"></a> [parameters](#input\_parameters) | Arbitrary parameters to configure the Backup Recovery Service instance. The value must be a map of key-value pairs. Conflicts with `parameters_json`. For service-specific parameters, refer to the IBM Cloud documentation. | `map(string)` | `null` | no |
| <a name="input_parameters_json"></a> [parameters\_json](#input\_parameters\_json) | Arbitrary parameters as a JSON string to configure the Backup Recovery Service instance. Conflicts with `parameters`. Use this when you need to pass complex nested structures. | `string` | `null` | no |
